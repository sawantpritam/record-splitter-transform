@RecordSplitter
Feature: RecordSplitter transform - Verify File source data transfer using RecordSplitter transform

  @FILE_SOURCE_TEST @BQ_SINK_TEST
  Scenario: To verify data is getting transferred from File to BigQuery sink plugin successfully with record splitter
    ...
    Then Connect plugins: "File" and "RecordSplitter" to establish connection
    ...
    Then Enter input plugin property: "path" with value: "recordSplitterGCSCsvFile"
  ...
  @FILE_SOURCE_TEST @BQ_SINK_TEST @PLUGIN-1276
  Scenario: To verify data is getting transferred from File to BigQuery sink plugin successfully with record splitter braces delimiter
    Given Open Datafusion Project to configure pipeline
    When Select plugin: "File" from the plugins list as: "Source"
    When Expand Plugin group in the LHS plugins list: "Transform"
    When Select plugin: "Record Splitter" from the plugins list as: "Transform"
    ...
    Then Navigate to the properties page of plugin: "RecordSplitter"
    Then Enter input plugin property: "fieldToSplit" with value: "recordSplitterFieldToSplit"
    Then Enter input plugin property: "delimiter" with value: "recordSplitterOpenBracesDelimiter"
    Then Enter input plugin property: "outputField" with value: "recordSplitterOutputField"
    Then Select Record Splitter plugin output schema action: "clear"
    Then Enter Record Splitter plugin outputSchema "recordSplitterValidOutputSchema"
    Then Validate "RecordSplitter" plugin properties
